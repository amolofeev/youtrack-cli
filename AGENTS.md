...
- Сверка со спекой (§4.8/DoR): если живой сервер недоступен — использовать снапшот
  `.opencode/openapi.json` (сверить версию API = 2025.3; файл gitignored, обновляется
  перекачиванием с живого сервера), зафиксировать отклонение в комментарии задачи;
  финальная сверка — Атом 8.4.
