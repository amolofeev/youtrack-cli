...
- Ручной смоук-тест против временного мок-сервера — опциональная верификация
  (поведение уже покрыто httptest-тестами), фикстуры и timestamps копируй из
  существующих тестов. При расхождении («decode response» и т.п.) сначала
  проверяй сам мок (путь без query, отдаёмое тело), а не дебагь CLI.
  Фоновый запуск мок-сервера в opencode bash-tool: голый `python3 mock.py &`
  вешает сессию (ждёт закрытия stdout) — используй
  `setsid bash -c 'python3 mock.py >/tmp/opencode/mock.log 2>&1 </dev/null' &`;
  останов — `pkill -f 'mock_yt[.]py'` (паттерн с `[.]` не совпадает с
  собственной командной строкой шелла).
