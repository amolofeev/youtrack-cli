...
- Верификация поведения (смоук/интеграция) — ТОЛЬКО против локального реального
  ...
  Токен для смоука: сначала проверь сохранённый конфиг `~/.config/yt/config.yml`
  (его читает сам yt) — токен оттуда использовать можно; спрашивать у человека,
  только если токена нет (не выдумывать, не хардкодить).
